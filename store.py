...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    Inspecting the live schema (PRAGMA table_info) rather than tracking a
    version counter makes running this twice a no-op by construction.

    Every ALTER is wrapped because _connect() has no lock of its own; two
    ...
    actually adds a column runs that column's backfill, so a backfill never
    runs twice.
    """
    cols = {r["name"] for r in conn.execute("PRAGMA table_info(sessions)")}
    # DEFAULT 'owner' is load-bearing: every session row predating this
    # column could only have been created by presenting AGENT_SECRET, so
    # backfilling them as owner sessions is correct — and SQLite only
    # accepts ADD COLUMN ... NOT NULL when a non-null default is supplied.
    if "tier" not in cols:
        try:
            conn.execute("ALTER TABLE sessions ADD COLUMN tier TEXT NOT NULL DEFAULT 'owner'")
